    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel("预试报告 (总览)",
                 ...
                 h4("4. 项目质量评估与结论 (Item Evaluation & Conclusion)"),
                 helpText("综合CTT与IRT指标的题目分析汇总表。建议结论基于区分度(RIT, a)与难度(Diff)生成，仅供参考。"),
                 tableOutput("item_conclusion_table"),
                 
                 h4("5. 项目参数详情 (IRT Parameters - Detials)"),
                 helpText("基于GRM模型估算的详细参数。"),
                 tableOutput("grm_params_table"),
                 
                 h4("6. 描述性统计 (Descriptive Statistics)"),
                 tableOutput("desc_stats_table"),
                 
                 h4("7. 反应频数分布 (Response Frequency)"),
...
server <- function(input, output, session) {
  ...
  # Combined Item Summary with Conclusion
  item_conclusion_df <- reactive({
    req(scoredData(), fit_grm())
    
    # 1. CTT Stats
    ctt <- ShinyItemAnalysis::ItemAnalysis(scoredData())
    # ctt has columns: Item, Diff, Avg. score, SD, ULI, RIT, RIR, Alpha Drop...
    
    # 2. IRT Stats
    coefs <- coef(fit_grm(), simplify = TRUE)
    if(is.list(coefs) && "items" %in% names(coefs)) {
      irt <- as.data.frame(coefs$items)
      irt$Item <- rownames(irt)
    } else {
      irt <- data.frame(Item = ctt$Item)
    }
    
    # Merge
    # Note: ItemAnalysis usually returns Item column.
    if(!"Item" %in% names(ctt)) ctt$Item <- rownames(ctt) # Fallback
    
    df <- merge(ctt, irt, by = "Item", all.x = TRUE)
    
    # 3. Logic for Conclusion
    # Rules:
    # - Delete: RIT < 0.2 (Low discrimination)
    # - Modify: 0.2 <= RIT < 0.3 OR (d_k extreme?) OR a < 0.8
    # - Keep: Otherwise
    
    df$Conclusion <- apply(df, 1, function(row) {
      rit <- as.numeric(row["RIT"])
      a_param <- as.numeric(row["a1"]) # Assuming 'a1' is the slope/discrimination
      ...
      if(rit < 0.2) {
        return("删除 (Delete)")
      } else if (rit < 0.3 || (!is.na(a_param) && a_param < 0.8)) {
        return("修改 (Modify)")
      } else {
        return("保留 (Keep)")
      }
    ...
    # Prioritize: Item, Conclusion, RIT, a1, Diff
    cols <- c("Item", "Conclusion", "RIT", "Diff", "ULI", "a1")
    # Add thresholds (d1, d2...) if they exist
  ...
  output$item_conclusion_table <- renderTable({
    req(item_conclusion_df())
    item_conclusion_df()
  })
  ...
  output$validity_scatter <- renderPlot({
    ...
    trig <- analysisTrigger()
    validate(
      need(trig$criterion != "None", "请在左侧选择效标变量(Criterion)，并点击“开始分析”按钮。")
    )
  ...
  wright_map_obj <- reactive({
    ...
    # Robustly calculate thresholds (b) for Wright Map
    # Use standard parameters (slope/intercept) to calculate b = -d/a
    # This works for 2PL, 3PL (location), and GRM (thresholds)
    
    coefs <- coef(fit, simplify = TRUE)
    
    # Handle list vs matrix return
    if (is.list(coefs) && "items" %in% names(coefs)) {
      pars <- coefs$items
    } else {
       return(NULL)
    ...
    # Identify a and d parameters
    nms <- colnames(pars)
    # Slopes (a1, a2...) - usually just a1 for unidimensional
    a_cols <- grep("^a", nms)
    # Intercepts (d1, d2...)
    d_cols <- grep("^d", nms)
    
    if (length(d_cols) == 0) return(NULL)
    
    # If slope is missing (e.g. some restricted Rasch parametrizations?), default to 1
    # But mirt usually returns a1=1 for Rasch.
    if (length(a_cols) > 0) {
      a <- pars[, a_cols[1]]
    } else {
      a <- rep(1, nrow(pars))
    ...
    # fscores
    fs <- tryCatch(as.vector(mirt::fscores(fit, method = "EAP")), error = function(e) NULL)
  ...
  output$downloadReport <- downloadHandler(
    ...
    content = function(file) {
      ...
      snap_conclusion <- tryCatch(item_conclusion_df(), error=function(e) NULL)
      ...
      params <- list(
        ...
        item_conclusion = snap_conclusion
      )
